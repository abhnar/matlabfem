   ...
   methods (Access = private)
       ...
       function WaitbarUpdate(Obj)
           ...
           EstimatedTimeNeeded = (UsedTime_now-Obj.UsedTime_1)/Obj.ReportInterval*(Obj.TotalMessage-Obj.NumMessage);
            mnts = EstimatedTimeNeeded/60;
           
           waitbar(Obj.NumMessage/Obj.TotalMessage, Obj.WaitbarHandle, [num2str(Obj.NumMessage/Obj.TotalMessage*100, '%.2f'), '%; ', num2str(UsedTime_now, '%.2f'), 's used and ', num2str(mnts, '%.2f'), 's needed.']);
       ...
       function FileUpdate(Obj)
           UsedTime_now = toc(Obj.StartTime);
           EstimatedTimeNeeded = (UsedTime_now-Obj.UsedTime_1)/Obj.ReportInterval*(Obj.TotalMessage-Obj.NumMessage);           
           mnts = round(EstimatedTimeNeeded/60,0);
           secs = rem(round(EstimatedTimeNeeded,0),60);
           switch Obj.FileName
               case 'screen'
                   fprintf('%.2f%%; %.2fs used and %dm%ds needed...\n', Obj.NumMessage/Obj.TotalMessage*100, UsedTime_now,  mnts,secs);
               otherwise
                   fprintf(Obj.FileID, '%.2f%%; %.2fs used and %dm%ds needed...\n', Obj.NumMessage/Obj.TotalMessage*100, UsedTime_now, mnts,secs);
           end
           Obj.UsedTime_1 = UsedTime_now;
       end
       function AddOne(Obj)
