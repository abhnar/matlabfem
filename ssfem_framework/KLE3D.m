classdef KLE3D < handle
    ...
    methods
        function ret = getKLEData(kle, corlenfrac)
            ...
            kle.CORR = kle.KLDATA.CORR{ix};
            kle.KLDATA.corlenfactor = corlenfrac;
            ret =  kle.CORR;
            
            
        end
        function process(kle, fem, domain)
...
%              kle.evaluate_Phi();
        ...
        function evaluate_Phi(kle, varargin)
            ...
            fprintf('Evaluation Eigen Function \n');
            a = kle.KLDATA.supSet.Co(:,:,1);
            b = kle.KLDATA.supSet.Co(:,:,2);
            c = kle.KLDATA.supSet.Co(:,:,3);
            d = kle.KLDATA.supSet.Co(:,:,4);
            
            
            ix1 = kle.KLDATA.supSet.ix1;
            nds = kle.KLDATA.supSet.nds;
            tet = kle.KLDATA.supSet.tet;
            V = kle.KLDATA.supSet.V;
            
            for it = 1:length(kle.KLDATA.CORR)
                normset = kle.KLDATA.CORR{it}.Norms;
                llim = min(length(kle.KLDATA.CORR{it}.Lambda), varargin{2});
                fprintf('for fraction %f\n',kle.crfracs(it));
                Phi = {};
                for j = 1: llim
                    eigvec = kle.KLDATA.CORR{it}.Evec(:,j);
                  
                    phi_centroid=zeros(kle.KLDATA.supSet.NTg,1);
                    for i=1:length(tet)
                        gix = ix1(i);
                        lix = i;
                        Xc=mean(nds(tet(lix,:),:));
...
%             obj.MD=ME.readNodalMesh(fname);
      ...
      function ret =calcEig_Multiple(obj,n)
            ...
            for it = 1:length(obj.Cmat)
                ...
                for i=1:n	    
                   a = obj.checkorthogonality(i,i);
                   obj.CORR{it}.Norms(i) = sqrt(a);
